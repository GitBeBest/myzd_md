...
function strRandomLong($length = 10) {
    ...
    if ($length > $len_per_loop) {
        ...
        for ($i = 0; $i < $multiplier; $i++) {
            $random_string .= substr(str_shuffle(md5(rand())), 0, $len_per_loop);
        }
        $random_string .= substr(str_shuffle(md5(rand())), 0, $remainder);
    } else {
        $random_string = substr(str_shuffle(md5(rand())), 0, $length);
    }
