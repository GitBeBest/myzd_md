...
class StatCode {
    ...
    const BK_TYPE_EXPERT_TEAM = 2; // 预约专家团队
    const BK_TYPE_DEPT = 3;       // 预约医院科室
    const BK_TYPE_QUICK_BOOK = 9;    // 快速预约
    const BK_TRAVEL_TYPE_PATIENT_GO = 1;    // 患者过去
    const BK_TRAVEL_TYPE_DOCTOR_COME = 2;   // 医生过来
    ...
    const BK_STATUS_SERVICE_PAID = 6;      // 已支付服务费（入院手术）上传出院小结
    ...
    const MR_REPORT_TYPE_MR = 'mr';    // 病历  Medical Record - PatientMRFile.reprot_type
    const MR_REPORT_TYPE_DA = 'dc';    // 出院小结  Discharge Certificate - PatientMRFile.reprot_type
    const USER_AGENT_WEBSITE = 'website';
    const USER_AGENT_WEIXIN = 'weixin';  //预约来源为微信
    const USER_AGENT_MOBILE_WEB = 'wap'; // 手机网站
    const USER_AGENT_APP_ANDROID = 'android';
    const USER_AGENT_APP_IOS = 'ios';
    const APP_NAME_MYZD = 'myzd'; // 患者版
    const APP_NAME_MOBILE_DOCTOR = 'md';   // 医生版
    const HAS_REMOTE = 1;   //已上传至七牛
    const HAS_NOT_REMOTE = 0; //已上传至七牛 未上传至七牛
    const DOCTOR_ACCEPT_AGREE = 1; //医生同意接受订单
    const DOCTOR_ACCEPT_NOT_AGREE = 0; //医生同意接受订单
...
// gender.
    ...
    public static function getOptionsBookingType() {
        return array(
            ...
            self::BK_TYPE_EXPERT_TEAM => '专家团队',
            self::BK_TYPE_DEPT => '医院科室',
            self::BK_TYPE_QUICK_BOOK => '快速预约'
        );
    }
    ...
    public static function getOptionsBookingTravelType() {
        return array(
            self::BK_TRAVEL_TYPE_PATIENT_GO => '邀请医生过来',
            self::BK_TRAVEL_TYPE_DOCTOR_COME => '希望转诊治疗'
        );
    }
    ...
    public static function getOptionsBookingStatus() {
        return array(
            ...
            self::BK_STATUS_SERVICE_PAID => '已支付服务费', // 已支付服务费（入院手术）
