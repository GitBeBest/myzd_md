...
class AuthSmsVerify extends BaseActiveRecord {
    ...
    public function rules() {
        return [
            [['mobile, code, action_type, time_expiry'], 'required'],
            [['mobile, action_type, is_active'], 'numerical', 'integerOnly' => true],
            [['mobile'], 'length', 'max' => 11],
            [['code'], 'length', 'is' => 6],
            [['id, mobile, code, action_type, time_expiry, is_active, date_created, date_updated, date_deleted'], 'safe', 'on' => 'search']
        ];
    }
    /**
    ...
    public static function getErrorMessage($code) {
        $msg = self::getAttributeLabel('code');
        switch ($code) {
    ...
    /**
     * @param bool|true $deactivateExpired 冻结该记录,如果已过期
     * @param bool|false $deactivate 冻结所有属于该action_type 的记录,如果没过期
     */
    ...
    public function isActionValid($actionType) {
        return ($actionType == self::ACTION_USER_REGISTER ||
            $actionType == self::ACTION_USER_PASSWORD_RESET ||
            $actionType == self::ACTION_BOOKING ||
            $actionType == self::ACTION_USER_LOGIN);
    }
    ...
    public function deActivateAllRecords() {
        $now = new \Datetime();
        $nowDate = $now->format(self::DB_FORMAT_DATETIME);
        $this->updateAllByAttributes(
            [
            'is_active' => 0,
